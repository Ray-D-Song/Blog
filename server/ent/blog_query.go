...
func (bq *BlogQuery) QueryRemarks() *RemarkQuery {
	query := (&RemarkClient{config: bq.config}).Query()
...
func (bq *BlogQuery) Clone() *BlogQuery {
	...
	return &BlogQuery{
		...
		withRemarks: bq.withRemarks.Clone(),
...
func (bq *BlogQuery) WithRemarks(opts ...func(*RemarkQuery)) *BlogQuery {
	query := (&RemarkClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withRemarks = query
	return bq
}
...
func (bq *BlogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Blog, error) {
	...
	if query := bq.withRemarks; query != nil {
		if err := bq.loadRemarks(ctx, query, nodes,
			func(n *Blog) { n.Edges.Remarks = []*Remark{} },
			func(n *Blog, e *Remark) { n.Edges.Remarks = append(n.Edges.Remarks, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BlogQuery) loadRemarks(ctx context.Context, query *RemarkQuery, nodes []*Blog, init func(*Blog), assign func(*Blog, *Remark)) error {
	...
	query.Where(predicate.Remark(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(blog.RemarksColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.blog_remarks
		if fk == nil {
			return fmt.Errorf(`foreign-key "blog_remarks" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blog_remarks" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
